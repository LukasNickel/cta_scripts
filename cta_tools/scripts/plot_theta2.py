...
def main(input_pattern, output, min_energy, max_energy):
    ...
    energy_key = 'ENERGY' if input_pattern.endswith('fits.gz') else 'gamma_energy_prediction'
    run_tables = []
    live_time = 0
    runs = []
    for f in input_files:
        print(f)
        t = read_table(f)
        mask = (t[energy_key].to_value(u.TeV) > min_energy) & (t[energy_key].to_value(u.TeV) < max_energy)
        cuts = read_cuts(f)
        if cuts is not None:
